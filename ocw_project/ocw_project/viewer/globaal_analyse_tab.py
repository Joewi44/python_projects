...
def create_globaal_analyse_tab(gdf_result: pd.DataFrame=None):
    ...
    def update_graph(scenario):
        subset = gdf_result[gdf_result["scenario_nm"] == scenario]
        ...
        category_order = list(shared_state.ocw_model.kwaliteitsindex.to_dict().keys())[::-1]

        # Ensure the DataFrame columns follow this order
        df_counts = df_counts.reindex(columns=category_order, fill_value=0)  # Reorder columns
    ...
    return pn.Column(
        pn.pane.Markdown(f"## Globale analysis for {gdf_result['gemeente'].unique()[0]}"),
        pn.Row(shared_state.scenario_selector),
        update_graph
    )